...
async function main() {
  ...
  const work = await fyredb.tenants.create({ name: 'Work', meta: {} });
  const personal = await fyredb.tenants.create({ name: 'Personal', meta: {} });
  ...
  console.log('\n=== Loading Work Tenant ===');
  await fyredb.tenants.open(work.id);
  console.log(`Active tenant: ${fyredb.tenants.activeTenant?.name}`);

  const taskRepo = fyredb.repo(TaskDef);
  ...
  await fyredb.tenants.open(personal.id);
  console.log(`Active tenant: ${fyredb.tenants.activeTenant?.name}`);
